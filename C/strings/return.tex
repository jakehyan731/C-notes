...
\IFRU{Первый вариант очень прост. Например}{The first option is very simple. For example}:
...
\\
\IFRU{В git в hex.c}{In hex.c of git}\footnote{\url{https://github.com/git/git/blob/master/hex.c}} 
...
объявление её как \TT{static} внутри ф-ции просто напросто
обеспечивает доступ к ней только из этой ф-ции}{In fact, the string is returned via global variable,
\TT{static} definition makes it visible only from this function}.
