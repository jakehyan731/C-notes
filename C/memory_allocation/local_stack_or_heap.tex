...
\IFRU{Когда я писал дизассемблер на Си (я делал это после того как длительное время писал на ЯП 
...
{When I was writing disassembled in C (I did it after a long period of programming in higher level PL ~---
...
{Aesthetically it looks good, in style of high-level PL, aside from that, such code is easier to read}.
...
\IFRU{и скопировать её куда-то, где она должен храниться}{and copy it to the some place where it should be
stored}.
