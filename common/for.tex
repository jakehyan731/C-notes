...
\IFRU{Но к сожалению, определять счетчики или \glslink{iterator}{итераторы} вместе с типами в теле самого for() вот так нельзя}
{However, it is not possible to declare \glslink{iterator}{iterators} with its types in for() clause}:
